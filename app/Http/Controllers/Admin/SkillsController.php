...
class SkillsController extends Controller
{
    ...
    public function showSkillsData(Request $request)
    {
        try{
            ...
            $validator = Validator::make($request->all(), [
                ...
                'proficiencyFilter' => 'nullable|string',
            ...
            //     $query->where('category', 'like', '%' . $request->input('categoryFilter') . '%');
            // }
            // if ($request->has('proficiencyFilter')) {
            //     $query->where('proficiency', 'like', '%' . $request->input('proficiencyFilter') . '%');
            // }
            // if ($request->has('orderFilter')) {
            //     $query->where('order', 'like', '%' . $request->input('orderFilter') . '%');
            // }
            // if ($request->has('statusFilter')) {
            //     $query->where('status', 'like', '%' . $request->input('statusFilter') . '%');
            // }

            // Sorting
            $orderableColumns = ['id', 'name', 'category', 'proficiency', 'order', 'status', 'created_at'];
                ...
                'data' => $data->map(function ($item) use ($isSingleResult) {
                    ...
                    $icon = $item->icon
                        ? sprintf(
                            ...
                            asset('storage/' . $item->icon),  // Link href
                            basename($item->icon), // Caption (using basename to get filename)
                            asset('storage/' . $item->icon)  // Image src
                        )
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request->hasFile('icon')) {
                $iconPath = $this->uploadFile($request->file('icon'), 'skills/icons');
                $skill->icon = $iconPath;
                $skill->save();
            }
            ...
            return response()->json([
                'message' => 'Error occurred while creating skill'
            ], 500);
        }
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            try {
                ...
                $skill = Skill::findOrFail($validated['id']);
            ...
            \Log::error('Error occurred while updating skill: ' . $e->getMessage(), [
                'skill_id' => $request->id,
                'error' => $e->getTraceAsString()
            ]);
            
            return response()->json([
                'message' => 'Error occurred while updating skill: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function delete(Request $request) 
    {
        try {
            ...
            // Delete the record
            $skill = Skill::withTrashed()->findOrFail($validated['id']);
            $skill->delete();
            ...
            return response()->json([
                'message' => 'Error occurred while deleting skill'
            ], 500);
        }
    }
    ...
    public function restore(Request $request) 
    {
        try {
            ...
            return response()->json([
                'message' => 'Error occurred while restoring skill'
            ], 500);
        }
    }
